...
impl Searcher {
    ...
    pub fn from_bytes(data: Vec<u8>, cache_policy: CachePolicy) -> Result<Self> {
        let header_buf: &[u8; HEADER_INFO_LENGTH] = data[..HEADER_INFO_LENGTH]
            .try_into()
            .map_err(|_| Ip2RegionError::IoError(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                "xdb data too short for header",
            )))?;
        let header = Header::try_from(header_buf)?;
        debug!(?header, "Load xdb bytes with header");
    ...
    pub fn read_buf(&self, offset: usize, size: usize) -> Result<Cow<'_, [u8]>> {
        ...
        if let Some(data) = &self.data {
            return Ok(Cow::from(&data[offset..offset + size]));
        }

        if self.cache_policy.ne(&CachePolicy::FullMemory) {
            let filepath = self.filepath.as_deref().ok_or(Ip2RegionError::NoFilePath)?;
            debug!(?filepath, offset=offset, size=size, "Read buf without cache");
        ...
        match self.full_cache.get() {
            None => {
                debug!(?filepath, "Load full cache");
                let mut file = File::open(filepath)?;
                let mut buf = Vec::new();
